...
class GlassesFilter(CatFilter):
    ...
    def __init__(self):
        super().__init__()
        self.filter = cv2.imread("/".join([self.filter_dir, "glasses.png"]), -1)
    ...
    def place_filter(self, img, keypoints):
        ...
        w_new = 2*(right_eye[0] - left_eye[0])
        r = w_new / self.w
        h_new = int(r * self.h)
        if w_new > 0 and h_new > 0:
            glasses = cv2.resize(self.filter, (w_new, h_new))
        else:
        ...
        left_most = left_eye[0] - w_new // 4
        top_left = left_eye[1] - h_new // 2
